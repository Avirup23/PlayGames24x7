...
roll_choice = function(red_store,blue_store,red_skill,blue_skill,p1,roll,move,bonus) #function to choose roll out of the available dice rolls
{
  ...
  # movement for random bot
  if(p1_skill == 2 && turn_skip == 0)
  {
  ...
  # skilled movements for responsible pair
  if(p1_skill == 4)
  {
    for(j in 1:4)
    {
  ...
  # skilled movements for aggressive player
  else if(p1_skill == 1)
  {
    for(j in 1:4)
    {
  ...
  # movement of aggressive player if skilled movements are not possible
  if(p1_skill == 1)
  {
...
turn_simulate = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll, d = 3)
{
  ...
  {
    # considering the first move of an odd turn
    if(move %% 3 == 1)
    {
      if(rmove == bmove)
      {
        ...
        # considering all moves except the first one
        if(move != 1)
        {
...
finalMatrix <- foreach(i = 1:n_sim, .combine = rbind) %dopar% {
  tempMatrix = turn_simulate(3,3,rep(1,16),1,1,dice_roll(3)) #for particular strategy pair, can be configured. (4,4) indicates the strategy tuple
