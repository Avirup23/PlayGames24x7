rm(list = ls())
#code for simulation of 1 dice,2 player ludo game
...
roll_choice = function(red_store,blue_store,red_skill,blue_skill,p1,roll,move,bonus) #function to choose roll out of the available dice rolls
{
  ...
  if(p1_skill %in% c(2.5,3,3.5) && turn_skip == 0 && (move<(1.5*n)))
  {
    #print(roll)
    ...
    for(i in 1:4)
    {
      for(m in 1:length(roll))
      {
        ...
        {
          n2 = -280*(p1_skill)^2 + 1720*p1_skill - 2540
          if(p1 == 2)
          {
...
turn_simulate = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll, d = 3)#function to simulate the game
{
  ...
  for(move in initial_blue_move:(1.5*n))
  {
    if(move %% 3 == 1) #considering the first move of an odd turn
    {
      if(rmove == bmove)
      {
        ...
        if(move != 1)#considering all moves except the first one
        {
          ...
          temp = roll_choice(red_store[(move-1),],blue_store[(move-1),],red_skill,blue_skill,1,roll = dice_roll(1),move,0)
          ...
          for(i in 1:10)
          {
            if(bonus  %in% 1:2)
            {
              temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,dice_roll(1),move,0)
            ...
            {
              temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,dice_roll(1),move,bonus)
              red_store[move,] = temp1[1:8]
              blue_store[move,] = temp1[9:16]
              bonus = temp1[18]
              if(bonus == 2)
        ...
        }else #considering the first move
        {
          ...
          temp = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,roll = dice_roll(1),move,0)#(move)th row is used as (move-1) row does not exist here
          red_store[move,] = temp[1:8]
          blue_store[move,] = temp[9:16]
          roll_ch[move,1] = temp[17]
          token_ch[move,1] = temp[19]
      ...
      temp = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,roll = dice_roll(1),move,0)
      ...
      for(i in 1:10)
      {
        if(bonus  %in% 1:2)
        {
          temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,dice_roll(1),move,0)
        ...
        {
          temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,dice_roll(1),move,bonus)
          blue_store[move,] = temp1[1:8]
          red_store[move,] = temp1[9:16]
          bonus = temp1[18]
          if(bonus == 2)
    ...
    }else if(move %% 3 == 2)
    {
      ...
      {
        roll = dice_roll(1)#rolling dice in beginning of a turn
      }
    ...
    }else if(move %% 3 == 0)
    {
      ...
      if(rmove == bmove)
      {
        temp = roll_choice(red_store[(move-1),],blue_store[(move-1),],red_skill,blue_skill,1,roll = dice_roll(1),move,0)#(move-1)th row of the storing matrices are considered as for beginning of each move,the indices of the tokens at the end of the last move is considered
        red_store[move,] = temp[1:8]
        blue_store[move,] = temp[9:16]
        bonus = temp[18]
        roll_ch[move,1] = temp[17]
...
n = 16
#turn_simulate(2.5,3,rep(1,16),1,1,dice_roll(1))[,2:9]
...
simulate_game = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll,fixed_roll,n_sim)#function to simulate the game a required number of times
{
  ...
  bcount = n_sim - rcount
  #print(pt)
  return(c(store_temp,rcount,bcount,c(summary(pt)[c(1,3,4,6),],sd(pt[,1]),sd(pt[,2]))))
}

#for implementing parallel processing
set.seed(0)
n = 16 #(0.67 * no. of moves)
n_sim = 1000
cores = detectCores()
c1 <- makeCluster(cores[1] - 1)
registerDoParallel(c1)
finalMatrix <- foreach(i = 1:n_sim, .combine = rbind) %dopar% {
  tempMatrix = turn_simulate(3.5,2.5,rep(1,16),1,1,dice_roll(1))
  ...
  tempMatrix = cbind(tempMatrix[,1:9],red_ini,blue_ini,tempMatrix[,10:19],red_score,blue_score)[,c(1:9,28,29)]
...
fin = finalMatrix[seq(n1,nrow(finalMatrix),(n1+1)),10:11] #for storing results

#code snippet for storing outcomes of set I in n_sim games  
set.seed(0) 
k = 1
n = 24
skill = matrix(rep(c(0,1,4),3), byrow = T, nrow = 3)
n_sim = 10000
points_I_24 = matrix(0, nrow = n_sim, ncol = 18)
set.seed(0)
...
for(i in 1:3)
{
  for(j in 1:3)
  {
  red_score = numeric(length = n_sim)
  blue_score = red_score
for(n2 in 1:n_sim)
{
  g = turn_simulate(skill[i,i],skill[i,j],rep(1,16),1,1,dice_roll(1),1)[(1.5*n),2:9]
  red_score[n2] = sum(g[1:4] == 57)*56 + sum(g[1:4] - 1)
  blue_score[n2] = sum(g[5:8] == 57)*56 + sum(g[5:8] - 1)
}
points_I_20[,c((2*k-1),2*k)] = c(red_score,blue_score)
k = k + 1
print(c(i,j))
}
}
