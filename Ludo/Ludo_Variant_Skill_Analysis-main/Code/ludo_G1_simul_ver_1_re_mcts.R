...
# Roll Choice Function
roll_choice = function(red_store,blue_store,red_skill,blue_skill,p1,roll,move,bonus, n_token) #function to choose roll out of the available dice rolls
{
  #print(roll)
      ...
      for(m in 1:length(roll))
      {
        ...
        if((p1_index[i] + roll[m]) <= home_sq)
        {
          ...
          if(p1 == 2)
          {
            ...
            pt_temp[i,m] = simulate_game(2,2,c(p2_temp,p1_temp),(move + 1),(move + p1 - 1),temp_roll,1,n2,n_token)[(n2+2)]
          ...
          }else
          {
            ...
            pt_temp[i,m] = simulate_game(2,2,c(p1_temp,p2_temp),(move + 1),(move + p1 - 1),temp_roll,1,n2,n_token)[(n2+1)]
...
# Turn Simulate Function
turn_simulate = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll, n_token, d = 3)#function to simulate the game
{
  ...
  for(move in initial_blue_move:(1.5*n))
  {
    ...
    if(move %% 3 == 1) #considering the first move of an odd turn
    {
      if(rmove == bmove)
      {
        ...
        if(move != 1)#considering all moves except the first one
        {
          ...
          temp = roll_choice(red_store[(move-1),],blue_store[(move-1),],red_skill,blue_skill,1,roll,move,0,n_token = n_token)
          ...
          for(i in 1:10)
          {
            if(bonus  %in% 1:2)
            {
              temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,dice_roll(d)[1],move,0,n_token = n_token)
            ...
            {
              temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,dice_roll(d)[1],move,bonus,n_token = n_token)
              red_store[move,] = temp1[1:(2*n_token)]
              blue_store[move,] = temp1[(2*n_token+1):(4*n_token)]
              bonus = temp1[4*n_token + 2]
              if(bonus == 2)
        ...
        }else #considering the first move
        {
          ...
          temp = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,1,roll,move,0,n_token = n_token)#(move)th row is used as (move-1) row does not exist here
          red_store[move,] = temp[1:(2*n_token)]
          blue_store[move,] = temp[(2*n_token+1):(4*n_token)]
          roll_ch[move,1] = temp[4*n_token+1]
          token_ch[move,1] = temp[4*n_token + 3]
      ...
      temp = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,roll,move,0,n_token = n_token)
      ...
      for(i in 1:10)
      {
        if(bonus  %in% 1:2)
        {
          temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,dice_roll(d)[1],move,0,n_token = n_token)
        ...
        {
          temp1 = roll_choice(red_store[move,],blue_store[move,],red_skill,blue_skill,2,dice_roll(d)[1],move,bonus,n_token = n_token)
          blue_store[move,] = temp1[1:(2*n_token)]
          red_store[move,] = temp1[(2*n_token+1):(4*n_token)]
          bonus = temp1[4*n_token + 2]
          if(bonus == 2)
    ...
    }else if(move %% 3 == 0)
    {
      ...
      if(rmove == bmove)
      {
        temp = roll_choice(red_store[(move-1),],blue_store[(move-1),],red_skill,blue_skill,1,roll,move,0,n_token = n_token)#(move-1)th row of the storing matrices are considered as for beginning of each move,the indices of the tokens at the end of the last move is considered
        red_store[move,] = temp[1:(2*n_token)]
        blue_store[move,] = temp[(2*n_token+1):(4*n_token)]
        bonus = temp[4*n_token + 2]
        roll_ch[move,1] = temp[4*n_token + 1]
...
point_system = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll,n_token)#function to compute points scored from final game state
{
  store = turn_simulate(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll,n_token = n_token)
  n1 = 1.5*n
  store[n1,]
...
simulate_game = function(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll,fixed_roll,n_sim,n_token)#function to simulate the game a required number of times
{
  ...
  for(m in 1:n_sim)
  {
    ...
    store_sim = point_system(red_skill,blue_skill,initial_state,initial_red_move,initial_blue_move,initial_roll,n_token)
...
# Only change this portions-----------
board_size = 11 #(13, 11, 9, 7)
home_sq = 0
if(board_size == 13)
{
  home_sq = 57
...
n_sim = 1000
mcts = c("3" = 100, "3.1" = 50, "3.2" = 10)
player1 = c(3,3,3.1,3.1,3.2,3.2)
player2 = c(3.1,3.2,3,3.2,3,3.1)
...
for (i in 1:length(player1)) {
  ...
  handlers("txtprogressbar")  # You can try "progress", "rstudio", etc.
  
  # Progress bar with foreach
  finalMatrix <- with_progress({
    p <- progressor(steps = n_sim)
    
    foreach(i = 1:n_sim, .combine = rbind) %dopar% {
      p()  # Progress step
      
      tempMatrix = turn_simulate(red_skill, blue_skill, rep(1, 4 * n_token), 1, 1, dice_roll(3),n_token = n_token)
  ...
  write.csv(finalMatrix,file=paste("/Users/avirup-chakraborty/Desktop/internship-25/Ludo/Ludo_Variant_Skill_Analysis-main/Data/","FI-",mcts[as.character(red_skill)],"_vs_FI-",mcts[as.character(blue_skill)],"_size_",board_size,"_tokens_",n_token,"_moves_24.csv",sep = ""))
}
