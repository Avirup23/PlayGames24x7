...
class Node:
    ...
    def best_child(self, c_param=20):
        return max(
            ...
            key=lambda child: (child.value / (child.visits + 1e-4) + 
                              c_param * math.sqrt(math.log(self.visits) / (child.visits + 1e-4)))
        )
    
    def print_tree(self, level=0):
        if self.move_str == None:
            print("    " * level + f"root : {round(self.value/(self.visits+1.e-3))}" )
        else:
            print("    " * level + f"{self.move_str} : {round(self.value/(self.visits+1.e-3))} {round(self.value / (self.visits + 1e-4) + 20 * math.sqrt(math.log(self.parent.visits) / (self.visits + 1e-4)))}" )
        for child in list(self.children.values()):
            child.print_tree(level + 1)

class MCTSAgent(Player):
    def __init__(self, name, iterations, drop=True):
        super().__init__(name)
    ...
    def mv1(self, index, state:Game_state, wcj, first, rules=[('Pseq',3),('Iseq',3)],maxscore=80):
        ...
        for _ in range(self.iterations):
            ...
            # print("-----------------------------")
            # print(f"0 counter: {init_state.counter} player:{init_state.player_index}")
            # pprint_hand(init_state.hands[0])
            # pprint_hand(init_state.hands[1])
            # Selection
            ...
            # print(f"1 counter: {init_state.counter} player:{init_state.player_index}")
            # pprint_hand(init_state.hands[0])
            # pprint_hand(init_state.hands[1])
            # Expansion
            ...
            # print(f"2 counter: {init_state.counter} player:{init_state.player_index}")
            # pprint_hand(init_state.hands[0])
            # pprint_hand(init_state.hands[1])
            # Simulation
            ...
            while node.parent is not None:
                node.visits += 1
                node.value -= reward[player_order]
                node = node.parent
        ...
        if len(root.children) == 2:
            best_move = max(root.children.values(), key=lambda n: round(n.value/(n.visits+1.e-3))).move_str
        else:
            raise ValueError(f"len(root.children)!=2")

        # root.print_tree()
        # print(f"Round {state.counter//2+1} Player {state.player_index} : best move {best_move}")
        # exit(0)
        return best_move
