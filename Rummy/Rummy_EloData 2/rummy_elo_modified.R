...
# Apply moving average (window size = 20, adjust as needed)
df_long <- df_long %>%
  group_by(Series) %>%
  mutate(Moving_Avg = slide_dbl(Value, mean, .before = 2000, .after = 0, .complete = TRUE))

# Compute average values per series (for horizontal lines)
avg_lines <- df_long %>%
  group_by(Series) %>%
...
main_colors <- c(
  ...
  "Defeat Heuristic"    = "#ffe111"
)

# Plot
ggplot(df_long, aes(x = Games, y = Moving_Avg, color = Series)) +
  geom_line(size = 0.7, na.rm = TRUE) +
  geom_hline(data = avg_lines, aes(yintercept = Average, color = Series),
             size = 2, alpha = 0.5, show.legend = TRUE) +
  scale_color_manual(values = main_colors) +
  # ylim(1200, 1500) +
  labs(title = "Dynamic Plot of Strategy Ratings (Smoothed)",
       x = "Game Index",
       y = "Dynamic Ratings (Moving Average)",
       color = "Strategy") +
  theme(
